...
void neapolitan_love_slide (int i, int j, Hentai& pix )
{
    pix[i][j]=(255-pix[i][j]);
}
Gimp_Mask gimp_mask (size_t s)
{
    double val = 1. / (double)(s*s);
    return Gimp_Mask(s, s, val);
}

Hentai load_bitmap(const std::string& filename)
{
    ...
    printf("Successfully loaded a %lux%lu hentai - %s.\n\n", h, w, filename.c_str());

    Hentai image_array(header, BitmapInfo, h, w);
...
int save_bitmap(const std::string& filename, Hentai& image_array)
{
...
// tu trzeba zmienić tak aby kopiowało jawnie obraz i zwracało kopię obrazu
Hentai& prostating(Hentai& img, std::function<void(int, int, Hentai&)> fun){
...
void fisting(int i, int j, Hentai& img)
{
    int size_mask=3;
